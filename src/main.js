...
SearchResults.prototype.search = async function search() {
    ...
    if (anyResults) {
      this.render(`
                <div class="${CLASSES.item}">
                    ...
                    <p><a class="govuk-link" href="/lpa/search/${escapeHTML(searchTerm)}">View all results</a></p>
                </div>
            `);
    } else {
      this.render(`
  ...
  this.render(`
        ...
        <div class="${CLASSES.item} ${CLASSES.itemSummary}">
            ...
            <a class="govuk-link" href="/lpa/search/${escapeHTML(searchTerm)}" style="float:right">View all</a>
        </div>
    `);
